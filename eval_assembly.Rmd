...
barPlot(eval_tbl, eval_tbl$assembler, eval_tbl$misassemblies, fill=eval_tbl$assembler, xlab='Assembler', ylab='# Misassemblies')

barPlot(eval_tbl, eval_tbl$assembler, eval_tbl$genome_fraction, fill=eval_tbl$assembler, xlab='Assembler', ylab='Genome Fraction [%]')

```
```{r}
boxPlot <- function(data, x, y, fill, xlab, ylab){
  p <- ggplot(data=data, aes(x=x, y=y, fill=fill)) + 
    geom_boxplot() + 
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
    theme_bw() + 
